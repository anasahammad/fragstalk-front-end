...
const ProductsByBrands = () => {
  ...
  const {data =[]} = useQuery({
    queryKey: ['products'],
    queryFn: async()=>{
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/customer/products/brands/${id}`);
...
const {data:brands=[]} = useQuery({
  ...
  const filterOptions = {
    brand: brands?.length > 0 ? brands.map(item => item.name) : [],
    price: ['Under $50', '$50 - $100', '$100 - $200', 'Over $200'], // ব্র্যান্ড অপশন চাইলে API থেকে নিতে পারো
    };
    ...
    const filteredData = useMemo(() => {
      return data
        .filter(product => {
          // ক্যাটাগরি ফিল্টার
          if (selectedFilters.brand && selectedFilters.brand !== product.brand) {
            return false;
          }
