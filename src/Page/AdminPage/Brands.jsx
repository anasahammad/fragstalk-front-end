...
const Brands = () => {
  ...
  const handleSubmit = async (e, aroma, isEditing = false) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`${import.meta.env.VITE_API_URL}/brand/${aroma._id}`, aromaData);
      } else {
        await axios.post(`${import.meta.env.VITE_API_URL}/brand`, aromaData);
      }
      fetchCategory();
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/brand/${id}`);
      fetchCategory();
    } catch (error) {
  ...
  const handleToggleActive = async (aroma) => {
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/brand/${aroma._id}`, {
        ...aroma,
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={(e) => handleSubmit(e, editingCategory || newCategory, !!editingCategory)} className="mb-8 bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <h2 className="text-2xl font-bold mb-4">{editingCategory ? 'Edit Brand' : 'Add New Brand'}</h2>
        <div className="mb-4">
          <input
            type="text"
        ...
        >
          {isUploading ? 'Uploading...' : (editingCategory ? 'Update Brand' : 'Add Brand')}
        </button>
        {editingCategory && (
          <button
