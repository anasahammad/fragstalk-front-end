...
const UpdateProductPage = () => {
  ...
  const { fields: additionalInformation, append: appendAdditional, remove: removeAdditional } = useFieldArray({
    control,
    name: "additional"
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (productData) => {
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/admin/products/${id}`, productData, {headers: {Authorization: `Bearer ${user.token}`}});
      return response.data;
    },
    onSuccess: () => {
  ...
  const onSubmit = async (data) => {
    const imageUrls = await Promise.all(
      data.images.map(async (file) => {
        if (typeof file === 'string') return file; // Already uploaded image
